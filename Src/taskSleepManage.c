...
/*private prototypes*/
void taskSleepManage_task(void * pvParametersid);

void taskSleepManage_init(void){
	...
	xTaskCreate(
			taskSleepManage_task,       /* Function that implements the task. */
			"Sleep management task",          /* Text name for the task. */
...
void taskSleepManage_task(void * pvParameters ){

	int CurrentTimeMonth = 0;
	int CurrentTimeHour = 0;
	int CurrentTimeMinute = 0;

	xSleepSemaphore = xSemaphoreCreateBinary();
